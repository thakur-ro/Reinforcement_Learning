...
class EpsilonGreedy(BanditAgent):
    ...
    def choose_action(self):
        ...
        """
        action = argmax(self.Q) if np.random.random() > self.epsilon else np.random.randint(0,self.k) 
        return action
...
class UCB(BanditAgent):
    ...
    def choose_action(self):
        ...
        """
        if self.t < 10:
            choices = list(np.argwhere(self.N == 0).flatten())
            return np.random.choice(choices)
        else:
            value = self.Q + self.c * np.sqrt(np.divide(np.log(self.t),self.N))
            return argmax(value)
